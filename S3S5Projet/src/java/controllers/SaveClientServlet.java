 ...
 * @author tiavi
 */
@WebServlet(name = "SaveClientServlet", urlPatterns = {"/SaveClient"})
public class SaveClientServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            client.setNom(nom);
            client.setGenre_id(genre);
            client.save(null);
            response.sendRedirect("FormClient");
        } catch (Exception ex) {
            Logger.getLogger(SaveMeubleEmployeServlet.class.getName()).log(Level.SEVERE, null, ex);
            response.sendRedirect("FormClient?error=" + ex.getMessage());
        }
    }
